...
#----------WASI
        ...
        pai_scale_structure = {
            ...
            "Depression": ["Cognitive", "Affective", "Physiological", "Scores > 95T"],
            "Suicidal ideation": [],
            "Aggression": ["Aggressive attitude", "Verbal aggression", "Physical aggression"],
            "Paranoia": ["Hypervigilance", "Persecution", "Resentment"],
            "Schizophrenia": ["Psychotic Experiences >70T", "Psychotic Experiences >85T", "Social Detachment", "Thought Disorder"],
            ...
            "Nonsupport": ["Scores > 88T"],
            "Alcohol/Drug": ["Scores > 84T"]
        }
    ...
    def generate_pai_section(self):
        ...
        he_she, his_her, himself_herself, mr_ms = pronouns.get(title, ("they", "their", "themself", "The client"))
        ...
        # ... all your logic, but instead of building one string, append to paragraphs list

        paragraphs.append(f"{pai_type}.")  # first paragraph

        if valid == "no":
            selected = [k for k, v in self.validity_checks.items() if v.get()]
            if not selected:
                paragraphs.append(f"{mr_ms} {last_name}'s PAI results were invalid.")
            else:
                for scale in selected:
                    if scale == "Infrequency":
                        paragraphs.append(f"{mr_ms} {last_name} did not attend appropriately to item content in responding to the PAI items.")
                    elif scale == "Negative Impression":
                        paragraphs.append(
                            f"{mr_ms} {last_name} presented an extremely negative evaluation of {his_her}self and {his_her} life. "
                    ...
                    elif scale == "Positive Impression":
                        paragraphs.append(f"{mr_ms} {last_name} attempted to portray {his_her}self as exceptionally free of the common shortcomings to which most individuals will admit.")
            return paragraphs

        # If valid
        paragraphs.append(
            f"Results of the PAI are valid. {mr_ms} {last_name} responded consistently and there is no indication that "
            ...
            for label, var in subs.items():
                if var.get():
                    ...
                    if domain in PAI_PARAGRAPHS and subkey in PAI_PARAGRAPHS[domain]:
                        ...
                        para = para.replace("[Mr./Ms. Patient last name]", f"{mr_ms} {last_name}")
                        para = para.replace("Herself", himself_herself.capitalize())
                        para = para.replace("herself", himself_herself)
                        para = para.replace("Her", his_her.capitalize())
                        para = para.replace("her", his_her)
                        para = para.replace("She", he_she.capitalize())
                        para = para.replace("she", he_she)
    ...
    def generate_report(self):
        ...
        tmt_percentile_raw = self.entries["tmt_percentile"].get().strip()
        ...
        tmt_percentile = ordinal(tmt_percentile_raw) if tmt_percentile_raw.isdigit() else tmt_percentile_raw

        patient = self.entries["patient"].get().strip()
        date = self.entries["date"].get().strip()
        ordered_by = self.entries["ordered_by"].get().strip()
        ...
        def yesno_text(var, true_text, false_text=""):
            return true_text if var.get() else false_text
        ...
        def generate_tmt_section():
            tmt_impairment = self.tmt_impairment_var.get()
            relative = yesno_text(self.tmt_relative_var, " relative to IQ.", ".")
            title = self.title_var.get()
            patient_name = self.entries["patient"].get()
            last_name = patient_name.split()[-1] if patient_name else "the client"

            if self.tmt_bottom10_var.get():
                return (
                    f"In the Trail Making Test (a test of speed for visual search, attention, and mental flexibility), "
                    f"{title} {last_name} performed in the bottom 10th percentile, "
                    f"showing {tmt_impairment} impairment{relative}"
                )

            tmt_percentile = self.entries["tmt_percentile"].get()
            if tmt_percentile and tmt_impairment:
                return (
                    f"In the Trail Making Test (a test of speed for visual search, attention, and mental flexibility), "
        ...
        def generate_wasi_section():
            ...
            waa_relative = yesno_text(self.waa_relative_var, " relative to IQ")
            wm = self.wm_var.get()
            if waa and wm:
                return (
        ...
        def generate_lns_section():
            ...
            lns_relative = yesno_text(self.lns_relative_var, " relative to IQ.", ".")
...
# Add plain text contact info, centered
            ...
            self.add_paragraph(doc, "")
            self.add_paragraph(doc, generate_tmt_section())
            self.add_paragraph(doc, "")
            ...
            for para in self.generate_pai_section():
                if para.strip():  # skip empty
                    self.add_paragraph(doc, para.strip())
                    self.add_paragraph(doc, "")  # add space between paragraphs
            self.add_paragraph(doc, "")
            self.add_paragraph(doc, "")
